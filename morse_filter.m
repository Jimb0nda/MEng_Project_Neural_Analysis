function [eeg, emg, t, itpc_eeg, itpc_emg] = morse_filter(srate,dataR_eeg, dataR_emg,num_freq,frex)
...
for i = 1:num_freq
    ...
    % COI
    t(i) = t(i) + sqrt(2)*a*(sqrt(b*g)/Wbg);

    % compute ITPC
    itpc_eeg(i,:) =  exp(1i*angle(eeg_as));
    itpc_emg(i,:) =  exp(1i*angle(emg_as));
